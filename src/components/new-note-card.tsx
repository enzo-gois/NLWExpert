    ...
    <Dialog.Root>
      <Dialog.Trigger className="rounded-md flex flex-col bg-slate-700 text-left p-5 gap-3 hover:ring-2 outline-none hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400">
        ...
        <p className="text-sm leading-6 text-slate-400">
          Grave uma nota de aúdio que será convertida para texto automaticamente
        </p>
      </Dialog.Trigger>
          ...
          <form className="flex-1 flex flex-col">         
            <div className="flex flex-col flex-1 gap-3 p-5">
              ...
              {shouldShowOnboarding ? (
                ...
                <textarea 
                  autoFocus 
                  className="text-sn leading-6 text-slate-400 bg-transparent resize-none flex-1 outline-none" 
            ...
            {isRecording ? (
              <button type="button" onClick={handleStopRecording} className="w-full flex items-center justify-center gap-2 bg-slate-900 py-4 text-center text-sn text-slate-300 outline-none font-medium">
                <div className="size-3 rounded-full bg-red-500 animate-pulse"/>
                Gravando! (clique p/interromper)
              </button>
            ): (
              <button type="button" onClick={handleSaveNote} className="w-full bg-lime-400 py-4 text-center text-sn text-lime-950 outline-none font-medium hover:bg-lime-500">
                Salvar nota
              </button>
            )}
          </form> 
